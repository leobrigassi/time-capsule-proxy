...
if ! which docker-compose >/dev/null 2>&1; then
  echo "Docker-compose is required for this script to work correctly. Please install and try again."
  exit 1
fi
if ! which smbclient >/dev/null 2>&1; then
  echo "smbclient is required for this script to work correctly. Please install and try again."
  exit 1
fi
...
while ! sudo docker-compose logs | grep -q "Welcome to Alpine Linu"; do
...
sed -i 3d $TCP_SERVICE_MOUNT_FILE
sed -i '3isource '$TCP_ENV' #line_3_updated_on_first_run' $TCP_SERVICE_MOUNT_FILE
fi
